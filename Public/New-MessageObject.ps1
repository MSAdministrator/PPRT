        ...
        Get-ChildItem $Message | ForEach-Object -Process {
            ...
            if ($msgFn -like "*.msg") {
                ...
                else
                {
                    ...
                    {
                        'Subject' { $Obj.Subject = $msg.Subject}
                        'Body'                       { $Obj.Body                       = $msg.body}
                        'HTMLBody'                   { $Obj.HTMLBody                   = $msg.HTMLBody}
                        'BCC'                        { $Obj.BCC                        = $msg.BCC}
                        'CC'                         { $Obj.CC                         = $msg.CC}
                        'ReceivedOnBehalfOfEntryID'  { $Obj.ReceivedOnBehalfOfEntryID  = $msg.ReceivedOnBehalfOfEntryID}
                        'ReceivedOnBehalfOfName'     { $Obj.ReceivedOnBehalfOfName     = $msg.ReceivedOnBehalfOfName}
                        'ReceivedTime'               { $Obj.ReceivedTime               = $msg.ReceivedTime}
                        'Receipents'                 { $Obj.Receipents                 = $msg.Receipents}
                        'ReplyRecipientsName'        { $Obj.ReplyRecipientsName        = $msg.ReplyRecipientsName}
                        'SenderName'                 { $Obj.SenderName                 = $msg.SenderName}
                        'SentOnDate'                 { $Obj.SentOnDate                 = $msg.SentOnDate}
                        'SentOnBehalfOfName'         { $Obj.SentOnBehalfOfName         = $msg.SentOnBehalfOfName}
                        'SentTo'                     { $Obj.SentTo                     = $msg.SentTo}
                        'SenderEmailAddress'         { $Obj.SenderEmailAddress         = $msg.SenderEmailAddress}
                        'SenderEmailType'            { $Obj.SenderEmailType            = $msg.SenderEmailType}
                        'SendUsingAccount'           { $Obj.SendUsingAccount           = $msg.SendUsingAccount}
                        'Header'                    { $Obj.Header                     = $header }
                        'Attachments'                { $Obj.Attachments                = $AllAttachments}
                    }
