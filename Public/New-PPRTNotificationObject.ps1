...
function New-PPRTNotificationObject
{
    ...
    Begin
    {
        if ($null -eq $SMTPServer)
        {
            ...
            {
                Write-Error -Message 'Your $PSEmailServer variable is null.  You must either set this variable or provide a SMTP Server address.'
                Write-Error -Message 'Exiting.....'
                return
            }
        }
    }
    Process
    {
        foreach ($msg in $MessageObject)
        {
            ...
            Send-MailMessage -To "$($Splat -join ' ')"
        }      
    }
    End
    {
