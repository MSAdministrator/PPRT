...
param (
    ...
    Begin
    {
        try
        {
            $TestingWebRequest = Invoke-WebRequest -Uri 'https://www.google.com' | Out-Null
        }
        catch
        {
    ...
    Process
    {
        try
        {
            $ipdata = Invoke-RestMethod -Uri "http://rdap.arin.net/bootstrap/ip/$ipaddress" -UseBasicParsing
        }
        catch
        {
